...
1. Copy the source to /usr/src:
   ```bash
   sudo cp -r /path/to/ethernet-linux-ixgbe /usr/src/ixgbe-6.2.6
   ```

2. Add the module to DKMS:
   ```bash
   sudo dkms add -m ixgbe -v 6.2.6
   ```

3. Build the module:
   ```bash
   sudo dkms build -m ixgbe -v 6.2.6
   ```

4. Install the module:
   ```bash
   sudo dkms install -m ixgbe -v 6.2.6
   ```

### Method 2: One-step installation

If the source is already in `/usr/src/ixgbe-6.2.6/`:
```bash
sudo dkms install -m ixgbe -v 6.2.6
```
...
1. Check DKMS status:
   ...
   ```
   ixgbe/6.2.6, 5.15.0-generic, x86_64: installed
   ```
...
1. Uninstall the module:
   ```bash
   sudo dkms uninstall -m ixgbe -v 6.2.6
   ```

2. Remove from DKMS:
   ```bash
   sudo dkms remove -m ixgbe -v 6.2.6 --all
   ```

3. Optionally, remove the source:
   ```bash
   sudo rm -rf /usr/src/ixgbe-6.2.6
   ```
...
```bash
sudo dkms build -m ixgbe -v 6.2.6 -k <kernel-version>
sudo dkms install -m ixgbe -v 6.2.6 -k <kernel-version>
```
...
```bash
cat /var/lib/dkms/ixgbe/6.2.6/build/make.log
```
